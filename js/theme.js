...
(function () {
  ...
  /** Initialize dynamic Mobile Hamburger Menu and Sidebar Drawer */
  function initMobileNav() {
    const headerContainer = document.querySelector('.site-header .container');
    const siteNav = document.querySelector('.site-nav');
    if (!headerContainer || !siteNav) return;

    // 1. Create and inject hamburger button
    const hamburger = document.createElement('button');
    hamburger.className = 'nav-hamburger';
    hamburger.id = 'nav-hamburger';
    hamburger.setAttribute('aria-label', 'Toggle Navigation Menu');
    hamburger.innerHTML = '<span></span><span></span><span></span>';
    headerContainer.insertBefore(hamburger, siteNav.nextSibling);

    // 2. Create Overlay and Sidebar elements
    ...
    // Copy links from site-nav (excluding theme-toggle containers)
    let linksHtml = '';
    ...
    sidebar.innerHTML = `
      <div class="mobile-sidebar-header">
        <span style="font-weight:700; font-size:1.15rem; color:var(--color-text-primary);">Menu</span>
        <button class="mobile-sidebar-close" id="mobile-sidebar-close">&times;</button>
      </div>
      <nav class="mobile-sidebar-nav">
        ${linksHtml}
      </nav>
      <div class="mobile-sidebar-footer">
        <div class="theme-toggle" id="mobile-theme-toggle" style="justify-content:center;"></div>
      </div>
    `;
    ...
    // 3. Initialize theme toggle inside the sidebar footer
    const mobileThemeContainer = document.getElementById('mobile-theme-toggle');
    ...
    // 4. Attach Event Triggers
    const toggleMenu = (forceState) => {
      const active = (typeof forceState === 'boolean') ? forceState : !sidebar.classList.contains('active');
      sidebar.classList.toggle('active', active);
      overlay.classList.toggle('active', active);
      hamburger.classList.toggle('active', active);
      document.body.style.overflow = active ? 'hidden' : ''; // prevent background scroll
    };
